      ...
      try {
        ...
        if (!document.querySelector(`link[href*="${newTheme}.css"]`)) {
          if (newTheme !== DEFAULT_THEME || !document.documentElement.classList.contains(DEFAULT_THEME)) {
            ...
            link.onload = () => log.info({ page: "ThemeContext", component: "ThemeProvider", func: "applyTheme" }, `Theme CSS loaded: ${newTheme}.css`);
            link.onerror = () => log.error({ page: "ThemeContext", component: "ThemeProvider", func: "applyTheme" }, `Error loading theme CSS: ${newTheme}.css`);
